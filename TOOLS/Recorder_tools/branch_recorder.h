...
/*		printf("host_domID:%d host_cr3:%lx vpmu_data:%lx\n",
	...
	void set_ept(int fd, int _guest_domID, int _host_domID, unsigned long _host_cr3, unsigned long _va){
		privcmd_hypercall_t hyper1 = { 
			__HYPERVISOR_vt_op, 
			{ 5, _guest_domID, _host_domID, _host_cr3, _va}
		};  
		ioctl(fd, IOCTL_PRIVCMD_HYPERCALL, &hyper1);		
	}
...
unsigned long pre_from;
void go_through_bts(Bts_record *bts, unsigned long start, unsigned long end, FILE *fd){
	unsigned long ptr = start;
	Bts_record *bts_buffer = bts;
	while(ptr < end){
//		fprintf(fd, "From:%lx To:%lx               ptr:%lx end:%lx\n", bts_buffer->from, bts_buffer->to, ptr, end);
		if(bts_buffer->from != pre_from){
			fprintf(fd, "From:%lx To:%lx\n", bts_buffer->from, bts_buffer->to);
		}
		pre_from = bts_buffer->from;
		bts_buffer += 1;
		ptr += BTS_RECORD_SIZE;
	}
}
