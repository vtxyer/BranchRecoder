...
#if GUEST_PAGING_LEVELS >= 4 /* 64-bit only... */
uint64_t
set_1g_guest_tables(struct vcpu *v, unsigned long cr3, walk_t *gw)
{
    struct domain *d = v->domain;
...
//	int ret;
	...
	void *top_map;
	unsigned long page_table_gfn_base;
	
	
	/*over physical region 1G*/
	guest_physical_addr = v->domain->extra_gfn[0];
	guest_physical_addr <<= 12;
	page_table_gfn_base = (v->domain->max_pages) + 1;
	page_table_gfn_base <<= 12;
	...
	if(!top_page){
		printk("<VT> set_1g_guest_tables error gfn\n");
		return -1;
	}
	...
	va = 0xffff880000000000 + guest_physical_addr;
    ...
    gflags = guest_l3e_get_flags(gw->l3e) ^ iflags;
    if ( (gflags & _PAGE_PRESENT) ) {
		printk("<VT> set_1g_guest_tables already set level 3 entrys error\n");
        goto out;
    }
...
/*	if(!guest_supports_1G_superpages(v)){
	...
	entry_val = page_table_gfn_base | flags;
